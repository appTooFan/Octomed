 ...
 * sound.mp3, book.pdf, review.html, video.json
 * books/*, screenshot/*
 ...
 * - YouTube Iframe support reading from video.json
 *
 * Repo:
 * - owner: appTooFan
 * - repo : Rafigtalib
...
const GH = {
  ...
  cdnBase: "https://rafigtalib.vercel.app"
};
 ...
 * @param {string} path e.g "0/0/0/books"
 */
...
// ✅ دالة ذكية لتحويل أي رابط يوتيوب إلى رابط Embed صالح للـ Iframe
...
async function loadLessonFromGitHub(s,u,l){
  ...
  const quizHtml = cdnUrl(`${base}/quiz.html`);
  // ✅ قراءة روابط اليوتيوب من ملف video.json بدلا من جلب الملفات
  let videos = [];
  try{
    const jsonUrl = cdnUrl(`${base}/video.json`);
    ...
    //  
      ...
      videos = data.map(v => ({
        url: getYouTubeEmbedUrl(v.url)
      }));
    }
  }catch(e){ videos = []; console.log("video.json not found or invalid format"); }
  ...
  return { base, audioSrc, doctorPdf, reviewHtml, videos, books, shots ,quizHtml};
}
...
function openLessonView(subjectIndex, unitIndex, lessonIndex, lessonTitle){
  ...
  const hash = window.location.hash;
      const lessonMatch = hash.match(/^#\/subject\/(\d+)\/\units\/(\d+)\/lesson\/(\d+)$/);
      if(!lessonMatch)
  {
    history.pushState({page:"lesson"},"",`#/subject/${subjectIndex}/units/${unitIndex}/lesson/${lessonIndex}`)
  ...
  // ✅ استبدال <video> بـ <iframe> لليوتيوب
  change_Page.innerHTML = `
    <div class="view_videos">
      <iframe id="videoPlayer" src="https://www.youtube.com/embed/ghi789" width="100%" height="100%" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
function initVideoNavigator(videos){
  ...
  function render(){
    ...
    if(player){ player.src = videos[index].url; } // ✅ تغيير الرابط للـ Iframe بدلاً من load()
...
function initLessonButtons(data){
  ...
  if(recoderBtn){
    recoderBtn.addEventListener("click", () => {
      const hash = window.location.hash;
      const recoderMatch = hash.match(/^#\/recoder\$/);
      if(!recoderMatch)
  {
    history.pushState({page:"recoder"},"",`#/recoder`)
  ...
  if(doctorBtn){
    doctorBtn.addEventListener("click", () => {
      const hash = window.location.hash;
      const doctorBookMatch = hash.match(/^#\/doctorBook\$/);
      if(!doctorBookMatch)
  {
    history.pushState({page:"doctorBook"},"",`#/doctorBook`)
  ...
  if(otherBtn){
    otherBtn.addEventListener("click", () => {
      const hash = window.location.hash;
      const otherBooksMatch = hash.match(/^#\/otherBooks\$/);
      if(!otherBooksMatch)
  {
    history.pushState({page:"otherBooks"},"",`#/otherBooks`)
...
const quizBtn = qs(".view_Data_Lesson .interactive_quiz");
if(quizBtn){
      ...
      if(!quizMatch)
  {
    history.pushState({page:"quiz"},"",`#/quiz`)
      ...
      softNavigate(() => openQuiz(data.quizHtml), 300);
    });
  }
}
...
function openQuiz(url){
  ...
  if(!url){ alert("quiz.html غير موجود"); return; }
  classremove();
  hide(".proFile");
  change_Page.innerHTML = "";
  change_Page.classList.add("quiz_view");
...
function route() {
     ...
     // renderSubjectsHome()
      ...
      if (lessonMatch) {
        setFooterActive(0);
        openLessonView(+lessonMatch[1],+lessonMatch[2],+lessonMatch[3])
        return;
      }
      if(addsMatch){
        setFooterActive(1);
